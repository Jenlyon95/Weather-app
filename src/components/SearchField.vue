...
const handleSearch = () => {
    ...
    searchTerm.timeout = setTimeout(async () => {
        if (searchTerm.query !== '') {
            const res = await fetch(`http://api.weatherapi.com/v1/search.json?key=33cf47a7a25645888ab152040240711&q=${searchTerm.query}`)
...
const getWeather = async (id) => {
    const res = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=33cf47a7a25645888ab152040240711&q=id:${id}&days=3&aqi=no&alerts=no`)
